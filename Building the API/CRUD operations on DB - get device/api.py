...
import dal  # DAL qatını modul səviyyəsində istifadə edirik
...
def device(identifier):
    if request.method == 'GET':
        ...
        # mövcud cihazı güncəllə
        updated = dict(db[identifier])
        updated.update(args)
        db[identifier] = updated
        db.sync()
...
def device_inventory():
    ...
    try:
        ...
        # Burada dict qaytarmaq lazımdır, "jsonify('ValidationError:', ...)" düzgün deyil
        return jsonify(err.messages), 400
